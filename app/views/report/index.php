<main>
    <div class="container-fluid px-4">
        <h5 class="mt-4 fw-bold" style="border-bottom: 1px solid black;">Report</h5>
        <div class="card mb-4 mt-4">
            <div class="card-header">
                <div class="row g-3 align-items-end justify-content-between">
                    <!-- Filter Form -->
                    <form action="<?= BASE_URL . '/report' ?>" method="post" class="col-lg-9 row g-2">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <label for="endDate" class="form-label">Date Thru</label>
                            <input type="date" id="endDate" name="endDate" class="form-control" required
                                value="<?= $endDate ?? '' ?>">
                        </div>
                        <div class="col-md-4 d-flex">
                            <button type="submit" class="btn btn-primary w-100 mt-auto" id="submitBtn" disabled>Tampilkan Data</button>
                        </div>
                    </form>

                    <?php if (!empty($report)): ?>
                        <div class="col-lg-3 d-flex justify-content-end">
                            <form action="<?= BASE_URL . '/report/downloadReport' ?>" method="post">
                                <input type="hidden" name="startDate" value="<?= $startDate ?>">
                                <input type="hidden" name="endDate" value="<?= $endDate ?>">
                                <button class="btn bg-cantik text-white button-cantik" id="downloadReportBtn" style="min-width: 150px;">
                    ...
                    <table id="example" class="display nowrap" style="width:100%">
                        <thead>
                            <tr class="bg-card text-white">
                                ...
                                <th class="small text-center">Jumlah SDM</th>
                                <th class="small text-center">Waktu Selesai</th>
                                <th class="small text-center">Durasi Bongkar</th>
                                <th class="small text-center">Poto</th>
                            </tr>
                        </thead>
                        <tbody id="reportTableBody">
                            <?php
                            extract($data);
                            require_once '../app/views/report/_partial_tabel_report.php';
                            ?>
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    function checkDates() {
        submitBtn.disabled = !(startInput.value && endInput.value);
    }
    ...
    function updateEndDateLimits() {
        ...
        if (endDate < startDate || endDate > lastDayOfMonth) {
            endInput.value = '';
        }
    }

    // Saat halaman dimuat, langsung aktifkan endDate jika startDate sudah terisi
    if (startInput.value) {
        updateEndDateLimits();
    }
